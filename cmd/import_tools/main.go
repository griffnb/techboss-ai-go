...
func init() {
	// Initialize the testing system to set up database connections
	system_testing.BuildSystem()
}

func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run main.go <path-to-tools.json>")
	}

	toolsJSONPath := os.Args[1]
...
func importTools(filePath string) error {
	...
	for i, tool := range tools {
		...
		if err != nil {
			fmt.Printf("Failed to import tool %s: %v\n", tool.ToolName, err)
...
func loadCategoryMaps() (map[string]string, map[string]string, error) {
	...
	allCategories, err := category.FindAll(ctx, nil)
	if err != nil {
	...
	for _, cat := range allCategories {
		...
		if cat.ParentCategoryID.Get() != "" {
			...
			if parentSlug == "categorizations" {
				categoryMap[slug] = cat.ID().String()
			} else if parentSlug == "business-functions" {
...
func importSingleTool(tool ToolData, categoryMap, businessFunctionMap map[string]string) error {
	...
	err := aiTool.Save(nil)
	if err != nil {
		return fmt.Errorf("failed to save AI tool: %w", err)
	}
