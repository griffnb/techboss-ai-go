...
- The model MUST ensure that each story be completable in ONE context window (~10 min of AI work). **Rule of thumb:** If you cannot describe the change in 2-3 sentences, it is too big.
...
- The model MUST format each task following this structure:
...
When creating the tasks.md file, if the implementation plan contains more than 3 tasks, the model MUST include instructions for using sub agents to complete the tasks with 1 agent per task at a time. with instructions to validate the sub agents work before marking the task complete.
...
- The model MUST include a "Task Delegation Process" with these requirements:
  ...
  5. Track Progress: Mark tasks as complete after sub-agent finishes and you have verified its quality
