...
func TestBuildPasswordWasResetEmail(t *testing.T) {
	accountObj := account.TESTCreateAccount()
	template, err := email_sender.BuildPasswordWasResetEmail(accountObj)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSendPasswordWasResetEmail(t *testing.T) {
