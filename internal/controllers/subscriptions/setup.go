//go:generate core_gen controller Subscription -modelPackage=subscription -skip=authCreate,authUpdate
...
const (
	TABLE_NAME string = subscription.TABLE
	ROUTE      string = "subscription"
)
...
func Setup(coreRouter *router.CoreRouter) {
	...
	coreRouter.AddMainRoute(tools.BuildString("/admin/", ROUTE), func(r chi.Router) {
		r.Group(func(adminR chi.Router) {
			...
			adminR.Get("/{id}/details", helpers.RoleHandler(helpers.RoleHandlerMap{
				constants.ROLE_READ_ADMIN: response.StandardRequestWrapper(adminDetails),
			}))
	...
	coreRouter.AddMainRoute(tools.BuildString("/", ROUTE), func(r chi.Router) {
		r.Group(func(authR chi.Router) {
			...
			authR.Get("/current", helpers.RoleHandler(helpers.RoleHandlerMap{
				constants.ROLE_ANY_AUTHORIZED: response.StandardPublicRequestWrapper(authCurrent),
			}))
		})
	})
}
