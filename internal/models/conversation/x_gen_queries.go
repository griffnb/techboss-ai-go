...
func FindAll(ctx context.Context, options *model.Options) ([]*Conversation, error) {
	return all[*Conversation](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Conversation, error) {
	return first[*Conversation](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Conversation, error) {
	return get[*Conversation](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*ConversationJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*ConversationJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*ConversationJoined, error) {
	AddJoinData(options)
	return first[*ConversationJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*ConversationJoined, error) {
	AddJoinData(options)
	return all[*ConversationJoined](ctx, options)
}
