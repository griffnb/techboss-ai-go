...
func FindAll(ctx context.Context, options *model.Options) ([]*Document, error) {
	return all[*Document](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Document, error) {
	return first[*Document](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Document, error) {
	return get[*Document](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*DocumentJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*DocumentJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*DocumentJoined, error) {
	AddJoinData(options)
	return first[*DocumentJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*DocumentJoined, error) {
	AddJoinData(options)
	return all[*DocumentJoined](ctx, options)
}
