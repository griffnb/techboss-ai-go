...
func FindAllRestrictedJoined(ctx context.Context, options *model.Options, sessionAccount coremodel.Model) ([]*SandboxJoined, error) {
...
func FindAllRestricted(ctx context.Context, options *model.Options, sessionAccount coremodel.Model) ([]*Sandbox, error) {
	// Uncomment and adjust the following lines to implement proper restrictions
	options.WithCondition("%s = :organization_id:", Columns.OrganizationID.Column())
	options.WithParam(":organization_id:", sessionAccount.GetString("organization_id"))
...
func GetRestrictedJoined(ctx context.Context, id types.UUID, sessionAccount coremodel.Model) (*SandboxJoined, error) {
...
func GetRestricted(ctx context.Context, id types.UUID, sessionAccount coremodel.Model) (*Sandbox, error) {
	options := model.NewOptions().
		WithCondition("%s.id = :id:", TABLE).
		WithParam(":id:", id).
		WithCondition("%s = :organization_id:", Columns.OrganizationID.Column()).
...
func NewPublic(data map[string]any, sessionAccount coremodel.Model) *Sandbox {
	obj := New()
	data = sanitize.SanitizeModelInput(data, obj, &Structure{})
	obj.MergeData(data)
	obj.AccountID.Set(sessionAccount.ID())
	obj.OrganizationID.Set(types.UUID(sessionAccount.GetString("organization_id")))
	return obj
}

func UpdatePublic(obj *Sandbox, data map[string]any, _ coremodel.Model) {
	data = sanitize.SanitizeModelInput(data, obj, &Structure{})
	obj.MergeData(data)
}
