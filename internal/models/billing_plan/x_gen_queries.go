...
func FindAll(ctx context.Context, options *model.Options) ([]*BillingPlan, error) {
	return all[*BillingPlan](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*BillingPlan, error) {
	return first[*BillingPlan](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*BillingPlan, error) {
	return get[*BillingPlan](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*BillingPlanJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*BillingPlanJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*BillingPlanJoined, error) {
	AddJoinData(options)
	return first[*BillingPlanJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*BillingPlanJoined, error) {
	AddJoinData(options)
	return all[*BillingPlanJoined](ctx, options)
}
