...
func FindAll(ctx context.Context, options *model.Options) ([]*ObjectTag, error) {
	return all[*ObjectTag](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*ObjectTag, error) {
	return first[*ObjectTag](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*ObjectTag, error) {
	return get[*ObjectTag](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*ObjectTagJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*ObjectTagJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*ObjectTagJoined, error) {
	AddJoinData(options)
	return first[*ObjectTagJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*ObjectTagJoined, error) {
	AddJoinData(options)
	return all[*ObjectTagJoined](ctx, options)
}
