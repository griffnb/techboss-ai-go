...
func FindAll(ctx context.Context, options *model.Options) ([]*AgentAttribute, error) {
	return all[*AgentAttribute](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*AgentAttribute, error) {
	return first[*AgentAttribute](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*AgentAttribute, error) {
	return get[*AgentAttribute](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*AgentAttributeJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*AgentAttributeJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*AgentAttributeJoined, error) {
	AddJoinData(options)
	return first[*AgentAttributeJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*AgentAttributeJoined, error) {
	AddJoinData(options)
	return all[*AgentAttributeJoined](ctx, options)
}
