...
const (
	UNIT_TEST_FIELD         = "price_or_plan_id"
...
func TestSave(t *testing.T) {
	...
	objFromDb, err := subscription.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedObjFromDb, err := subscription.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
...
func TestFindAll(t *testing.T) {
	...
	objs, err := subscription.FindAll(context.Background(), options)
	if err != nil {
...
func TestFindFirst(t *testing.T) {
	...
	obj2, err := subscription.FindFirst(context.Background(), options)
	if err != nil {
...
func TestFindFirstJoined(t *testing.T) {
	...
	options := &model.Options{
		Conditions: fmt.Sprintf("%s.id = :id:", subscription.TABLE),
		Params: map[string]any{
			":id:": obj.ID(),
		},
	}
	obj2, err := subscription.FindFirstJoined(context.Background(), options)
	if err != nil {
		t.Fatalf("Get Err %v", err)
	}
