...
func FindAll(ctx context.Context, options *model.Options) ([]*Organization, error) {
	return all[*Organization](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Organization, error) {
	return first[*Organization](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Organization, error) {
	return get[*Organization](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*OrganizationJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*OrganizationJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*OrganizationJoined, error) {
	AddJoinData(options)
	return first[*OrganizationJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*OrganizationJoined, error) {
	AddJoinData(options)
	return all[*OrganizationJoined](ctx, options)
}
