package billing_plan_price_test
...
func TestSave(t *testing.T) {
	...
	objFromDb, err := billing_plan_price.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedObjFromDb, err := billing_plan_price.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
...
func TestFindAll(t *testing.T) {
	...
	objs, err := billing_plan_price.FindAll(context.Background(), options)
	if err != nil {
...
func TestFindFirst(t *testing.T) {
	...
	obj2, err := billing_plan_price.FindFirst(context.Background(), options)
	if err != nil {
...
func TestFindFirstJoined(t *testing.T) {
	...
	options := &model.Options{
		Conditions: fmt.Sprintf("%s.id = :id:", billing_plan_price.TABLE),
		Params: map[string]any{
			":id:": obj.ID(),
		},
	}
	obj2, err := billing_plan_price.FindFirstJoined(context.Background(), options)
	if err != nil {
		t.Fatalf("Get Err %v", err)
	}
