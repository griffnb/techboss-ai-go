...
func FindAll(ctx context.Context, options *model.Options) ([]*Agent, error) {
	return all[*Agent](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Agent, error) {
	return first[*Agent](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Agent, error) {
	return get[*Agent](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*AgentJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*AgentJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*AgentJoined, error) {
	AddJoinData(options)
	return first[*AgentJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*AgentJoined, error) {
	AddJoinData(options)
	return all[*AgentJoined](ctx, options)
}
