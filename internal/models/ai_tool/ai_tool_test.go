...
func TestSave(t *testing.T) {
	...
	objFromDb, err := ai_tool.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedObjFromDb, err := ai_tool.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
...
func TestFindAll(t *testing.T) {
	...
	objs, err := ai_tool.FindAll(context.Background(), options)
	if err != nil {
...
func TestFindFirst(t *testing.T) {
	...
	obj2, err := ai_tool.FindFirst(context.Background(), options)
	if err != nil {
...
func TestFindFirstJoined(t *testing.T) {
	...
	options := &model.Options{
		Conditions: fmt.Sprintf("%s.id = :id:", ai_tool.TABLE),
		Params: map[string]any{
			":id:": obj.ID(),
		},
	}
	obj2, err := ai_tool.FindFirstJoined(context.Background(), options)
	if err != nil {
		t.Fatalf("Get Err %v", err)
	}
