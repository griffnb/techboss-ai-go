...
func TestSave(t *testing.T) {
	...
	objFromDb, err := category.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedObjFromDb, err := category.Get(context.Background(), obj.ID())
	if err != nil {
		t.Fatal(err)
	}
...
func TestFindAll(t *testing.T) {
	...
	objs, err := category.FindAll(context.Background(), options)
	if err != nil {
...
func TestFindFirst(t *testing.T) {
	...
	obj2, err := category.FindFirst(context.Background(), options)
	if err != nil {
...
func TestFindFirstJoined(t *testing.T) {
	...
	options := &model.Options{
		Conditions: fmt.Sprintf("%s.id = :id:", category.TABLE),
		Params: map[string]any{
			":id:": obj.ID(),
		},
	}
	obj2, err := category.FindFirstJoined(context.Background(), options)
	if err != nil {
		t.Fatalf("Get Err %v", err)
	}
