...
func FindAll(ctx context.Context, options *model.Options) ([]*Lead, error) {
	return all[*Lead](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Lead, error) {
	return first[*Lead](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Lead, error) {
	return get[*Lead](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*LeadJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*LeadJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*LeadJoined, error) {
	AddJoinData(options)
	return first[*LeadJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*LeadJoined, error) {
	AddJoinData(options)
	return all[*LeadJoined](ctx, options)
}
