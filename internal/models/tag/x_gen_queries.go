...
func FindAll(ctx context.Context, options *model.Options) ([]*Tag, error) {
	return all[*Tag](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*Tag, error) {
	return first[*Tag](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*Tag, error) {
	return get[*Tag](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*TagJoined, error) {
	options := model.NewOptions().
		WithCondition("%s = :id:", Columns.ID_.Column()).
		WithParam(":id:", id)

	AddJoinData(options)
	return first[*TagJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*TagJoined, error) {
	AddJoinData(options)
	return first[*TagJoined](ctx, options)
}

// FindAllJoined Finds all records
func FindAllJoined(ctx context.Context, options *model.Options) ([]*TagJoined, error) {
	AddJoinData(options)
	return all[*TagJoined](ctx, options)
}
