...
func FindAll(ctx context.Context, options *model.Options) ([]*GithubInstallation, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
		return mocker.FindAll(ctx, options)
	}
	return all[*GithubInstallation](ctx, options)
}

func FindFirst(ctx context.Context, options *model.Options) (*GithubInstallation, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
		return mocker.FindFirst(ctx, options)
	}
	return first[*GithubInstallation](ctx, options)
}

func Get(ctx context.Context, id types.UUID) (*GithubInstallation, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
		return mocker.Get(ctx, id)
	}
	return get[*GithubInstallation](ctx, id)
}
...
func GetJoined(ctx context.Context, id types.UUID) (*GithubInstallationJoined, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
	...
	AddJoinData(options)
	return first[*GithubInstallationJoined](ctx, options)
}

// FindFirstJoined Finds first record
func FindFirstJoined(ctx context.Context, options *model.Options) (*GithubInstallationJoined, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
...
func FindAllJoined(ctx context.Context, options *model.Options) ([]*GithubInstallationJoined, error) {
	mocker, ok := model.GetMocker[*Mocker](ctx, PACKAGE)
	if ok {
		return mocker.FindAllJoined(ctx, options)
	}
	AddJoinData(options)
	return all[*GithubInstallationJoined](ctx, options)
}
