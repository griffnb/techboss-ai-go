...
func GetClaudeImageConfig() *ImageConfig {
	return &ImageConfig{
		...
		DockerfileCommands: []string{
			...
			// Create workspace directory (ownership will be fixed before Claude execution)
			...
			// Keep USER as root for exec commands (allows permission fixes)
			// We'll switch to claudeuser explicitly when running Claude
			"USER root",
		},
	}
}
